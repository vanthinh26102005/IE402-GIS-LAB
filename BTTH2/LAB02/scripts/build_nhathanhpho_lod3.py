...
SOURCE_PREFIX = WORKDIR / "data" / "shapefile" / "NhaHat_Footprint"
OUTPUT_GLTF = WORKDIR / "data" / "model" / "NhaHat_ThanhPho_LoD3.gltf"
OUTPUT_GEOJSON = WORKDIR / "data" / "geojson" / "NhaHat_ThanhPho_LoD3.geojson"
OUTPUT_GEOJSON_WGS84 = WORKDIR / "data" / "geojson" / "NhaHat_ThanhPho_LoD3_wgs84.geojson"
...
def main() -> None:
    ...
    rings_local = build_tiered_rings(local_points, total_height)
    
    # Generate WGS84 rings for ArcGIS SceneView
    wgs84_points = ensure_ccw(raw_points)
    rings_wgs84 = build_tiered_rings(wgs84_points, total_height)
    
    vertices, face_groups = build_mesh(rings_local)
    ...
    geojson_local = export_geojson(rings_local, total_height, name)
    geojson_wgs84 = export_geojson(rings_wgs84, total_height, name)

    OUTPUT_GLTF.write_text(json.dumps(gltf, ensure_ascii=False, indent=2), encoding="utf-8")
    OUTPUT_GEOJSON.write_text(json.dumps(geojson_local, ensure_ascii=False, indent=2), encoding="utf-8")
    OUTPUT_GEOJSON_WGS84.write_text(json.dumps(geojson_wgs84, ensure_ascii=False, indent=2), encoding="utf-8")

    print(f"Generated {OUTPUT_GLTF.name} with {len(vertices)} vertices and {len(all_faces)} triangles.")
    print(f"Generated {OUTPUT_GEOJSON.name} with {len(rings_local)} stacked footprint levels.")
    print(f"Generated {OUTPUT_GEOJSON_WGS84.name} with {len(rings_wgs84)} stacked footprint levels (WGS84).")
